<template>
  <span>
    <el-popover
      ...
      trigger="click"
    >
      <el-row v-if="clearable" type="flex" justify="end" class="d2-mb-10">
        <el-button
          type="danger"
          icon="el-icon-delete"
          size="mini"
      ...
      <el-collapse v-if="!searchMode" v-model="collapseActive" class="d2-icon-select--group">
        <el-collapse-item v-for="(item, index) in icon" :key="index" :title="item.title" :name="index" class="d2-icon-select--class">
          <el-row class="d2-icon-select--class-row">
            <el-col v-for="(iconName, iconIndex) in item.icon" :key="iconIndex" class="d2-icon-select--class-col" :span="4" @click.native="selectIcon(iconName)">
